    ...
    <div className="min-h-[100dvh] bg-[#fafafa] pb-16">
      ...
      <div className="mx-auto max-w-3xl px-4 pt-8 sm:px-6">
        ...
        <article className="mt-6 rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
          ...
          {displayShortDescription ? (
            <section className="mt-6">
              ...
              <p className="mt-2 text-sm leading-relaxed text-zinc-700">{displayShortDescription}</p>
            </section>
          ) : null}
        ...
        <section className="mt-8 rounded-2xl border border-zinc-200 bg-white p-5">
          <h2 className="text-base font-semibold text-zinc-900">Objevte další obsah na XXREALIT</h2>
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            <Link href="/nemovitosti" className="rounded-xl border border-zinc-100 bg-zinc-50 px-4 py-3 text-sm font-semibold text-zinc-800 hover:border-orange-200">
              Reality
            </Link>
            <Link href="/ubytovani" className="rounded-xl border border-zinc-100 bg-zinc-50 px-4 py-3 text-sm font-semibold text-zinc-800 hover:border-orange-200">
              Ubytování
            </Link>
            <Link href="/posts" className="rounded-xl border border-zinc-100 bg-zinc-50 px-4 py-3 text-sm font-semibold text-zinc-800 hover:border-orange-200">
              Příspěvky
            </Link>
            <Link href="/profesionalove" className="rounded-xl border border-zinc-100 bg-zinc-50 px-4 py-3 text-sm font-semibold text-zinc-800 hover:border-orange-200">
              Profesionálové a firmy
            </Link>
          </div>
          <div className="mt-4 flex flex-wrap gap-3 text-sm">
            <Link href="/firmy" className="font-semibold text-orange-700 hover:underline">
              Prohlédnout další firmy
            </Link>
            {company.city ? (
              <Link href={`/firmy?city=${encodeURIComponent(company.city)}`} className="text-zinc-600 hover:underline">
                Firmy ve městě {company.city}
              </Link>
            ) : null}
            {company.region ? (
              <Link href={`/firmy?region=${encodeURIComponent(company.region)}`} className="text-zinc-600 hover:underline">
                Firmy v kraji {company.region}
              </Link>
            ) : null}
          </div>
        </section>
