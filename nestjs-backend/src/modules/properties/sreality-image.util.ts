...
export function normalizeSrealityImageUrl(raw: string): string | null {
  ...
  if (!s) return null;
  if (s.startsWith('//')) return upgradeSrealityImageQuality(`https:${s}`);
  if (s.startsWith('http://') || s.startsWith('https://')) {
    return upgradeSrealityImageQuality(s);
  }
  if (s.startsWith('/')) return upgradeSrealityImageQuality(`https://img.sreality.cz${s}`);
  return null;
}
...
export function upgradeSrealityImageQuality(url: string): string {
  try {
    const u = new URL(url);
    let path = u.pathname;
    path = path.replace(/\/\d+x\d+\//g, '/full/');
