...
tým {{portalName}}`,
  ...
  },
  {
    ...
    category: 'company_directory',
    subject: '⭐ Na XXREALIT přibyla nová recenze vaší firmy',
    htmlContent:
      '<p>Dobrý den,</p><p>na portálu XXREALIT byla zveřejněna nová recenze společnosti <strong>{{companyName}}</strong>.</p><p><strong>Nové hodnocení:</strong><br/>{{reviewStars}} {{reviewAverage}} / 5</p><p><strong>Celkem recenzí:</strong> {{reviewCount}}</p><p><strong>Typ zkušenosti:</strong> {{sentiment}}</p><p><em>„{{reviewExcerpt}}“</em></p><p>Podívejte se, jak si vaše firma vede na XXREALIT.</p><p><a href="{{ctaUrl}}">Zobrazit profil a recenzi</a></p><p><a href="{{secondaryCtaUrl}}">{{secondaryCtaLabel}}</a></p>',
    textContent:
      'Na XXREALIT byla zveřejněna nová recenze společnosti {{companyName}}.\n\n{{bodyText}}',
